...
- **M3.4 Comfort pass** [HW][Quest] — cylinder size/distance/curvature adjustable and persisted; `XR_FB_composition_layer_settings` sharpening on. *Accept:* 12 pt terminal text readable at default placement; no double-image on head motion. **Status — partial:** sharpening + quality **super-sampling** + a **mipmapped cylinder swapchain** landed (super-sampling, not sharpening, is what carries legibility; no double-image confirmed). Adjustable + persisted geometry (grab-to-move) **deferred** in favour of M3.5. The pass surfaced a sharp-vs-shimmer ceiling on the composition-layer path that M3.5 exists to break.
- **M3.5 Spatial SDK client (evaluation)** [HW][Quest] — a second, *parallel* Quest client on the **Meta Spatial SDK**, sharing `client/core`/`proto`, with the desktop rendered into a curved `VideoSurfacePanel` (AMediaCodec decodes into the panel `Surface`; transport/session bridged via JNI). **Motivation:** the composition-layer client (ARCHITECTURE §6.2) cannot reach *anisotropic* filtering — the runtime owns the sampler — so at a compact angular size text is either sharp-but-shimmery (over-sampled → minification aliasing) or clean-but-soft (resolution-matched), never both. An app-rendered panel can filter anisotropically (ARCHITECTURE §6.5). *Accept:* live desktop on a curved Spatial SDK panel from `loomd`, side-by-side **sharper and shimmer-free** vs the OpenXR client at equal angular size — or a recorded finding that it is not, in which case the cylinder-layer design stands.

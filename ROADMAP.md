...
**Status (2026-07-12):** M1.1–M1.3 ✅ complete on the Mac (loopback). Both repos' `check.sh` pass from a clean clone, and `scripts/demo.sh` streams the synthetic pattern host→SDL client and prints live overlay numbers (e2e ≈ 18 ms, 0 % loss at 720p). M1.4–M1.5 await the Linux box + NVENC.
...
- **M1.4 Real capture, Linux** [HW][Linux] — xdg-desktop-portal ScreenCast of a *physical* monitor via PipeWire; SHM path first. **Contains spike R2:** attempt dmabuf on NVIDIA, record outcome in ARCHITECTURE §13. *Accept:* live desktop visible in SDL client on the Mac across the LAN; capture→encode handoff < 10 ms measured.
